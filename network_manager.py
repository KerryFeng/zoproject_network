...
age = pd.read_excel("Data\\basic_data.xlsx", encoding = "gbk")
date = "2020/9/30"
...
df3 = pd.merge(df2, age, on = "基金代码")

res1 = pd.DataFrame()
res2 = pd.DataFrame()

for the_age in [0, 1]: # 0:年轻 1：老将
    the_df = df3[df3["任职年限分类"] == the_age]
    # 模型1
    regression1 = regression(the_df["仓位变化"], [the_df["网络平均"], the_df["所有基金平均"]])
    coefficients1 = regression1.params
    pvalues1 = regression1.pvalues
    res1 = res1.append([{"fund_type": the_age, "const": coefficients1["const"], 
    ...
    res2 = res2.append([{"fund_type": the_age, "const": coefficients2["const"], 
