...
for date in date_list:
    ...
    regression1 = regression(df2["仓位变化"], [df2["网络平均"], df2["所有基金平均"]])
    coefficients1 = regression1.params
    pvalues1 = regression1.pvalues
    
    the_df1 = pd.DataFrame()
    
    the_df1["coefficients"] = coefficients1
    the_df1["pvalues"] = pvalues1
    
    res1[date] = the_df1
    
    regression2 = regression(df2["仓位变化"], [df2["网络平均"], df2["首次重仓网络平均"], df2["所有基金平均"]])
    coefficients2 = regression2.params
    pvalues2 = regression2.pvalues
    
    the_df2 = pd.DataFrame()
    
    the_df2["coefficients"] = coefficients2
    the_df2["pvalues"] = pvalues2
